...
function minChange(coins, amount, memo = {0:0, 1:1}) {
    if(n in memo) return memo[n];
    for(let i = 0; i < coins.length; i++){
      memo[amount] = minChange(coins, amount-coins[i], memo) + 1;
    }
    return memo[amount];
}
     //memo[5] = minChange(coins, 4, memo) + 1, minChange(coins, 3, memo) + 1, minChange(coins, 0, memo) + 1
        //memo[5] = [3, 3, 1]
     //memo[4] = minChange(coins, 3, memo) + 1, minChange(coins, 2, memo) + 1
        //memo[4] = [3, 2]
     //memo[3] = [minChange(coins, 2, memo) + 1, minChange(coins, 1, memo) +1 ]
        //memo[3] = [3,2]
     //memo[2] = minChange(coins, 1, memo) + 1 , minChange(coins, 0, memo) + 1
        //memo[2] = [2,1]
