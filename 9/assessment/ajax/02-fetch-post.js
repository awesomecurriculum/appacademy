...
async function addPerson() {
  const response = await fetch('http://example.com/api/people', {
    method: 'POST',
    body: JSON.stringify({ firstName: 'Maul', lastName: 'Kimmerman' }),
  });
  const data = await response.json();
  const people = data.people;
  const ele = document.getElementById('people-list-02');
  people.forEach(p => (ele.innerHTML += `<li>${p.lastName}, ${p.firstName}</li>`));
}
...
(function (exports) {
  try {
    ...
    exports.addPerson = () => {
      throw new Error('Cannot export addPerson');
    };
  }
})(exports);
