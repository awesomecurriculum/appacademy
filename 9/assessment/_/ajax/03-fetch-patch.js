...
async function updatePerson() {
  const response = await fetch('http://example.com/api/people/3', {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ lastName: 'Ball' }),
  });
  const person = await response.json();
  const listItem = document.getElementById('person-3');
  listItem.innerHTML = `${person.lastName}, ${person.firstName}`;
}
...
(function (exports) {
  try {
    ...
    exports.updatePerson = () => {
      throw new Error('Cannot export updatePerson');
    };
  }
})(exports);
